...
sentinel_log_type =  'GenericWeboookLog'
...
def main(req: func.HttpRequest, context: func.Context) -> func.HttpResponse:
     ...
     if ((generic_webhook_secret not in (None, '') and not str(generic_webhook_secret).isspace())):    
        hash_object = hmac.new(generic_webhook_secret.encode('utf-8'), msg=req.get_body(), digestmod=hashlib.sha256)
        expected_signature = "sha256=" + hash_object.hexdigest()
        if 'x-hub-signature-256' not in req.headers:
            return func.HttpResponse(
             "Generic webhook signature header is missing.",
             status_code=403
        )
        signature_header = req.headers['x-hub-signature-256']
        if not hmac.compare_digest(expected_signature, signature_header):
            return func.HttpResponse(
             "Generic webhook signature verification failed.",
             status_code=403
        )
     ...
     try:
        ...
        logging.info("Info: Generic webhook data connector execution completed successfully.")
        return func.HttpResponse(
      ...
      logging.error( "Error: Generic webhook data connector execution failed with an internal server error.")
      raise
